...
{
    protected array $proxy_filters = [];
    protected array $public_attributes = [];
    ...
    public function validated($key = null, $default = null): array
    {
        ...
        foreach ($this->proxy_filters as $key => $proxy) {
            if (key_exists('mediate', $proxy) && key_exists($proxy['mediate'], $params)) {
                $params[$key] = $params[$proxy['mediate']];
                unset($params[$proxy['mediate']]);
            }
        }
        if (key_exists('relations', $params)) {
            $new_relations = [];
            $relations = explode(',', $params['relations']);
            foreach ($relations as $relation) {
                $relation_config = $this->relations[$relation];
                if (!key_exists("with", $relation_config)) {
                    $new_relations[] = $relation;
                }
                if (key_exists("with", $relation_config) && $relation_config["with"]) {
                    $new_relations[] = $relation_config["with"];
                }
            }
            $params['relations'] = implode(",", $new_relations);
        }
        return $params;
    }
    ...
    {
        return [
            'fields' => ['sometimes', 'min:2', new FieldsRule($this->public_attributes)],
    ...
    protected function filterRules(): array
    {
        $params = [
            ...
            'sort' => ['sometimes', 'min:2', new SortRule($this->public_attributes)],
            'fields' => ['sometimes', 'min:2', new FieldsRule($this->public_attributes)],
            'relations' => ['sometimes', 'min:2', new RelationsRule($this->relations)],
        ];
        foreach ($this->public_attributes as $attribute => $data) {
            if (!key_exists('rules', $data)) {
                $params[$attribute] = ['sometimes'];
            }
            if (key_exists('rules', $data)) {
                $query_param = $this->input($attribute);
                if (is_array($query_param)) {
                    $params[$attribute] = ['sometimes', 'array'];
                    foreach (Criteria::MAP as $key => $value) {
                        $params["$attribute.$key"] = array_merge(['sometimes', 'nullable'], $data['rules']);
                    }
                }
                if (!is_array($query_param)) {
                    $params[$attribute] = array_merge(['sometimes', 'nullable'], $data['rules']);
                }
            }
        }
        foreach ($this->proxy_filters as $attribute => $data) {
            $rules = ['sometimes'];
            if (key_exists('rules', $data)) {
                $rules = array_merge($rules, $data['rules']);
            }
            if (key_exists('mediate', $data)) {
