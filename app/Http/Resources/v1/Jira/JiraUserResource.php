...
class JiraUserResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        $this->init($request);
        $jiraTeam = $this->when(
            $this->relationLoaded('jiraTeams') && $this->depthLevel(),
            function () {
                return JiraTeamResource::collection($this->jiraTeams);
            },
            null
        );
        ...
        $response = [
             ...
             * @example atlassian
             */
            'jira_user_type' => $this->when($this->include('jira_user_type'), $this->jira_user_type),
            /**
             * @format  boolean
             * @example 1
             */
            'active' => $this->when($this->include('active'), (bool)$this->active),
        ];

        if ($jiraTeam) {
            $response['jira_teams'] = $jiraTeam;
        }
         ...
         */
        $response['created_at'] = $this->when($this->include('created_at'), $this->created_at);
        $response['updated_at'] = $this->when($this->include('updated_at'), $this->updated_at);
